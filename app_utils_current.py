...
def _get_blocked_domains_from_file(path):
...
def _process_cleanup_batch(db, URL, blocked_domains):
    """Iterates through URLs and deletes phishing entries."""
    removed_count = 0
...
def cleanup_phishing_urls():
    ...
    path = current_app.config.get('BLOCKED_DOMAINS_PATH')
    blocked_domains = _get_blocked_domains_from_file(path)
    if not blocked_domains:
        return

    from app.models import db, URL
    try:
        removed_count = _process_cleanup_batch(db, URL, blocked_domains)
        if removed_count > 0:
            db.session.commit()
    except Exception: # nosec B110
        try:
