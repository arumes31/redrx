...
def _is_phishing_url(url, blocked_domains):
    """Internal helper to check if a single URL domain or any suffix is in the blocked list."""
    if not url:
        return False
    try:
...
def cleanup_phishing_urls():
    ...
    try:
        ...
        for url_entry in urls:
            try:
                # Check main long_url
                is_phishing = _is_phishing_url(url_entry.long_url, blocked_domains)

                # Check rotate_targets if main is clean
                if not is_phishing and url_entry.rotate_targets:
                    is_phishing = any(
                        _is_phishing_url(target, blocked_domains)
...
def get_blocked_domains():
    ...
    global \
        ...
        _blocked_domains_mtime, \
        _blocked_domains_path, \
        _blocked_domains_ino
    ...
    if path and os.path.exists(path):
        try:
            ...
            if (
                _blocked_domains_cache is None
                or path != _blocked_domains_path
                or mtime != _blocked_domains_mtime
                or ino != _blocked_domains_ino
            ):
                with open(path, "r", encoding="utf-8") as f:
                    _blocked_domains_cache = {
            ...
            raise RuntimeError(
                f"Critical Security Failure: Unable to load blocked domains list: {e}"
            )
        ...
        if _blocked_domains_cache is not None:
            return _blocked_domains_cache
        raise RuntimeError(
            f"Critical Security Failure: Blocked domains list configured at '{path}' but file does not exist."
        )
...
def is_safe_url(target_url, blocked_domains_cache=None):
    ...
    blocked_domains = (
        blocked_domains_cache
        if blocked_domains_cache is not None
        else get_blocked_domains()
    )
    if blocked_domains:
        parts = domain.split(".")
        for i in range(len(parts)):
    ...
    if request:
        ...
        if cf_country:
            ...
            if _redis_client:
                try:
                    _redis_client.setex(
                        f"{_GEO_PREFIX}{ip}", _GEO_CACHE_TTL, cf_country
                    )
                except Exception:
                    pass
            return cf_country
