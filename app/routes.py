...
def _handle_index_short_code(form):
    """Handles custom code check or generation."""
...
def _get_index_times(form):
    """Handles scheduling and expiry logic."""
    start_at = None
    ...
    if form.expiry_hours.data is not None:
        if form.expiry_hours.data == 0 or form.expiry_hours.data > 8760:
            if not current_user.is_authenticated:
                error = "Please log in to create links longer than 1 year or permanent links."
            elif form.expiry_hours.data == 0:
                expires_at = None
            else:
...
def _validate_index_urls(form):
    """Handles safety checks for URLs and targets."""
...
def index():
    ...
    if form.validate_on_submit():
        ...
        rotate_list, error = _validate_index_urls(form)
        if error:
            flash(error, 'danger')
            return render_template('index.html', form=form)

        short_code, error = _handle_index_short_code(form)
        if error:
            flash(error, 'danger')
            return render_template('index.html', form=form)

        start_at, end_at, expires_at, error = _get_index_times(form)
        if error:
            flash(error, 'warning')
            return render_template('index.html', form=form)
        ...
        qr_data = get_qr_data_url(short_url, color=form.qr_color.data, bg=form.qr_bg.data, logo_img=logo_img)
