...
def _parse_rotate_targets(targets_data):
    if not targets_data:
        return None, None
    rotate_list = [u.strip() for u in targets_data.split(",") if u.strip()]
...
def _validate_device_targets(ios_url, android_url):
    if ios_url and not is_safe_url(ios_url):
        return "iOS target URL is blocked or invalid."
    if android_url and not is_safe_url(android_url):
        return "Android target URL is blocked or invalid."
    return None


def _validate_additional_urls(form):
    rotate_list, error = _parse_rotate_targets(form.rotate_targets.data)
    if error:
        return None, error

    error = _validate_device_targets(
        form.ios_target_url.data, form.android_target_url.data
    )
    if error:
        return None, error
...
def _combine_datetime(date_field, time_field):
    if date_field and time_field:
        return datetime.datetime.combine(date_field, time_field)
    return None


def _calculate_expiry(expiry_hours):
    if expiry_hours is None:
        return None, None

    if expiry_hours == 0 or expiry_hours > 8760:
...
def _process_url_timestamps(form):
    start_at = _combine_datetime(form.start_date.data, form.start_time.data)
    end_at = _combine_datetime(form.end_date.data, form.end_time.data)

    if start_at and end_at and end_at <= start_at:
        return start_at, end_at, None, "End time must be after start time"

    expires_at, error = _calculate_expiry(form.expiry_hours.data)
    return start_at, end_at, expires_at, error
...
def _check_anonymous_restriction():
    if (
        current_app.config.get("DISABLE_ANONYMOUS_CREATE")
        and not current_user.is_authenticated
    ):
        return "Please log in to shorten URLs."
    return None


def _get_form_processed_data(form):
    ...
    return short_code, rotate_list, start_at, end_at, expires_at, error


def _handle_form_submission(form):
    error = _check_anonymous_restriction()
    if error:
        flash(error, "warning")
    ...
    short_code, rotate_list, start_at, end_at, expires_at, error = (
        _get_form_processed_data(form)
    )
    if error:
        flash(error, "danger" if "End time" not in str(error) else "warning")
        return render_template("index.html", form=form)

    _create_url_from_form(
        form, short_code, long_url, rotate_list, start_at, end_at, expires_at
    )
    ...
    return short_url, qr_data, stats_url


def _create_url_from_form(
    form, short_code, long_url, rotate_list, start_at, end_at, expires_at
):
    new_url = URL(
...
def index():
    form = ShortenURLForm()
    short_url = qr_data = stats_url = None

    if form.validate_on_submit():
        res = _handle_form_submission(form)
        if isinstance(res, tuple):
            short_url, qr_data, stats_url = res
        else:
            return res
...
def _get_time_series_stats(url_id, cutoff, sqlite_format, pg_format, time_data):
    ...
    if dialect == "sqlite":
        time_counts = (
            db.session.query(
                func.strftime(format_str, Click.timestamp), func.count(Click.id)
            )
            .filter(Click.url_id == url_id, Click.timestamp >= cutoff)
            .group_by(func.strftime(format_str, Click.timestamp))
            .all()
        )
    else:
        time_counts = (
            db.session.query(
                func.to_char(Click.timestamp, format_str), func.count(Click.id)
            )
            .filter(Click.url_id == url_id, Click.timestamp >= cutoff)
            .group_by(func.to_char(Click.timestamp, format_str))
            .all()
        )
