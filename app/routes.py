...
def _handle_index_short_code(form):
    custom_code = (
...
def _validate_index_urls(form, long_url):
    if not is_safe_url(long_url):
        return None, "That destination URL is blocked for safety reasons."
...
def index():
    ...
    if form.validate_on_submit():
        ...
        # 1. Validate URLs
        rotate_list, err = _validate_index_urls(form, long_url)
        if err:
            flash(err, "danger")
            return render_template("index.html", form=form)

        # 2. Handle Short Code
        short_code, err = _handle_index_short_code(form)
        if err:
            flash(err, "danger")
            return render_template("index.html", form=form)

        # 3. Get Times
        start_at, end_at, expires_at, err = _get_index_times(form)
        if err:
            flash(err, "warning")
            return render_template("index.html", form=form)

        password_hash = (
            generate_password_hash(form.password.data) if form.password.data else None
        )
...
def link_password_auth(short_code):
    _ = URL.query.filter_by(short_code=short_code).first_or_404()
...
def stats(short_code):
    ...
    # Process Analytics
    clicks = (
        Click.query.filter_by(url_id=url_entry.id).order_by(Click.timestamp.asc()).all()
    )
    ...
    for click in clicks:
        click_time = (
            click.timestamp.replace(tzinfo=datetime.timezone.utc)
            if click.timestamp.tzinfo is None
            else click.timestamp
        )

        # Global stats (all time)
        country_data[click.country] = country_data.get(click.country, 0) + 1
        browser_data[click.browser or "Unknown"] = (
            browser_data.get(click.browser or "Unknown", 0) + 1
        )
        platform_data[click.platform or "Unknown"] = (
            platform_data.get(click.platform or "Unknown", 0) + 1
        )
        ...
        if rc.ip_address:
            ...
            if len(parts) == 4:
                ...
                if len(v6_parts) >= 2:
                    ...
                    rc.ip_anonymized = (
                        rc.ip_address[: rc.ip_address.find(":") + 1] + "xxxx"
                        if ":" in rc.ip_address
                        else "xxxx"
                    )
        else:
