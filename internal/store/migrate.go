...
var schema = []table{
	...
	{
		name: "pow_challenge_claims",
		columns: []column{
			{"challenge_hash", "VARCHAR(64) NOT NULL PRIMARY KEY", "VARCHAR(64) NOT NULL PRIMARY KEY"},
...
var indexes = []struct{ name, ddl string }{
	...
	{"idx_pow_claim_expiry", "CREATE INDEX IF NOT EXISTS idx_pow_claim_expiry ON pow_challenge_claims (expires_at)"},
}
